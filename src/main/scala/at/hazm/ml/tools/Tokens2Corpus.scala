...
object Tokens2Corpus extends App{
  ...
  Source.fromInputStream(new GZIPInputStream(new FileInputStream(src)), "UTF-8").getLines().map(_.split("\t", 2)).flatMap{
    case Array(_, terms) => terms.split("\\s+")
  }.grouped(100).foreach{ terms =>
    vocab.register(terms)
  }
